...
export class WalletService {
  ...
  async getBalance(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get wallet balance for ${ownerType}:${ownerId}`,
        error,
      );
  ...
  async bootstrapWalletCache(
    ...
    try {
      ...
      await this.redis
        .getClient()
        .set(balanceKey, wallet.balanceCents.toString());
      ...
      this.logger.error(
        `Failed to bootstrap wallet cache for ${ownerType}:${ownerId}`,
        error,
      );
    }
  ...
  async recordCharge(
    ...
    const {
      ...
      requestId,
      model,
      newBalanceFromLua: _newBalanceFromLua,
    } = params;
  ...
  async addBalance(params: WalletTopupParams): Promise<{ newBalance: bigint }> {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.walletLedger.create({
        data: {
          ...
          balanceAfter: wallet.balanceCents,
          stripePaymentId:
            referenceType === 'stripe_session' ? referenceId : null,
          description:
    ...
    try {
      ...
      this.logger.error(
        `Failed to update Redis cache for ${ownerType}:${ownerId}`,
        error,
      );
    ...
    this.logger.log(
      `Wallet top-up: ${ownerType}:${ownerId} +$${(amountCents / 100).toFixed(2)}`,
    );
  ...
  async rollbackRedis(
    ...
    try {
      await this.redis.getClient().incrby(balanceKey, amountCents);
      this.logger.warn(
        `Redis rollback: ${ownerType}:${ownerId} +${amountCents} cents`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to rollback Redis for ${ownerType}:${ownerId}`,
        error,
      );
    }
  }
  ...
  async reconcile(ownerType: 'user' | 'org', ownerId: string): Promise<void> {
    ...
    await this.redis
      .getClient()
      .set(balanceKey, dbWallet.balanceCents.toString());
