...
export class RefundService {
  ...
  private async ensureScriptLoaded(): Promise<string> {
    ...
    const sha = (await this.redis
      .getClient()
      .script('LOAD', this.refundLuaScript)) as string;
  ...
  async processRefund(context: RefundContext): Promise<RefundResult> {
    ...
    try {
      ...
      const luaResult = await this.executeRefundLua(
        ownerType,
        ownerId,
        requestId,
        amountCents,
      ...
      this.logger.error(
        `Refund failed for ${ownerType}:${ownerId} request ${requestId}`,
        error,
      );
      return {
        status: 'ERROR',
  ...
  private async executeRefundLua(
    ...
    // Build Redis keys
    const idempotencyKey = BILLING_KEYS.refundIdempotency(
      ownerType,
      ownerId,
      requestId,
    );
    const refundCountKey = BILLING_KEYS.refundCount(
      ownerType,
      ownerId,
      dateStr,
    );
    ...
    try {
      ...
      if (errorMessage.includes('NOSCRIPT')) {
        ...
        const result = await this.redis
          .getClient()
          .eval(this.refundLuaScript, keys.length, ...keys, ...args);
        return Number(result);
      }
      throw error;
    }
