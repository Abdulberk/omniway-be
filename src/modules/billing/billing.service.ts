...
export class BillingService {
  ...
  async chargeBilling(
    ...
    // Prepare Redis keys
    const allowanceKey = BILLING_KEYS.allowanceUsed(
      ownerType,
      ownerId,
      dateStr,
    );
    const walletKey = BILLING_KEYS.walletBalance(ownerType, ownerId);
    const idemKey = BILLING_KEYS.billingIdempotency(
      ownerType,
      ownerId,
      requestId,
    );
  ...
  async getDailyUsage(authContext: AuthContext): Promise<DailyUsageInfo> {
    ...
    try {
      ...
      const allowanceUsed = allowanceUsedStr
        ? parseInt(allowanceUsedStr, 10)
        : 0;
      ...
      this.logger.error(
        `Failed to get daily usage for ${ownerType}:${ownerId}`,
        error,
      );
