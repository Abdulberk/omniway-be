...
export class StripeService implements OnModuleInit {
  ...
  async createSubscriptionCheckout(
    ...
    const { userId, organizationId, planId, successUrl, cancelUrl, seatCount } =
      params;
    ...
    if (!plan || !plan.stripePriceId) {
      throw new BadRequestException(
        'Plan not found or does not have Stripe price',
      );
    }
    ...
    this.logger.log(
      `Created subscription checkout session ${session.id} for plan ${planId}`,
    );
  ...
  async createTopupCheckout(
    ...
    if (packageId) {
      ...
      throw new BadRequestException(
        'Either packageId or amountCents/creditCents is required',
      );
    }
  ...
  async createBillingPortal(
    ...
    this.logger.log(
      `Created billing portal session for customer ${stripeCustomerId}`,
    );
   ...
   */
  async extractOwnerFromMetadata(
